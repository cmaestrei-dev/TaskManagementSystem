...
users_list = ['juan', 'andrea', 'leon', 'andres', 'natalia', 'camilo', 'rusbel', 'johan']
...
categories_list = ['desarrollo', 'mercadeo', 'conceptualización', 'ejercicios']
...
task_data.each do |category_name, task_name, participant_set|
  participants_objects = participant_set.map do |participant_string|
    user_name, raw_role = participant_string.split(':')
  ...
  Task.create!(
    ...
    owner: owner,
    code: "#{owner.id}#{Time.now.to_i.to_s(36)}#{SecureRandom.hex(8)}", 
    participations: participants_objects
  )
end
